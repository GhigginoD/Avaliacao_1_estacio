"""
Questão 2:
João Papo-de-Pescador, homem de bem, comprou um microcomputador para controlar o
rendimento diário de seu trabalho. 
Toda vez que ele traz um peso de peixes maior que o estabelecido pelo 
regulamento de pesca do estado de São Paulo (50 quilos) deve pagar uma
multa de R$ 4,00 por quilo excedente. 
João precisa que você faça um programa que leia a
variável peso (peso de peixes) e calcule o excesso. 
Gravar na variável excesso a quantidade de quilos além do limite e 
na variável multa o valor da multa que João deverá pagar. 
Imprima os dados do programa com as mensagens adequadas.
"""

WEIGHT_LIMIT = 50
RATE = 4
fish_weight = int(input("Entre com o peso do peixe: "))
if fish_weight <= 50: print("Peso dentro do limite!")
else: 
    exceed_fish_weight = fish_weight - WEIGHT_LIMIT
    price_penality = RATE * exceed_fish_weight
    print("Peso fora do limite!")
    print(f"Peso excedido em {exceed_fish_weight}kg.")
    print(f"Valor da penalidade R${price_penality}.")